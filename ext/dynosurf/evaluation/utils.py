...
def eval_mesh(
    mesh, gt_mesh, thresholds=np.linspace(1.0 / 1000, 1, 1000), num_sample=100000
):
...
def load_mesh(mesh_file):
    with open(mesh_file, "r") as f:
        ...
        n_vertices, n_faces, _ = list(map(lambda x: int(x), str_file[1].split(" ")))
...
def eval_correspondences_mesh(mesh_files, pcl_tgt, project_to_final_mesh=False):
...
def eval_correspondences_mesh2(
    meshes, pcl_tgt, project_to_final_mesh=False, return_colored_pcd=False
):
    """Calculates correspondence score for meshes.
    ...
    mesh_pts_t0 = np.expand_dims(mesh_pts_t0.astype(np.float32), axis=0)
    ind, _ = get_nearest_neighbors_indices_batch(
        mesh_pts_t0, np.expand_dims(pcl_tgt[S], axis=0)
    )
    ind = ind[0].astype(int)
    # Nex time steps
    eval_dict = {}
    ...
    for i in range(len(pcl_tgt)):
        ...
        if project_to_final_mesh and i == (len(pcl_tgt) - 1):
            ...
            v_t = pcl_tgt[i][ind2[0]]
        distances = np.linalg.norm(v_t - pc_nn_t, axis=-1)
        eval_dict["l2 %d (mesh)" % i] = np.mean(distances).item()

        if return_colored_pcd:
            l2_loss = np.mean(distances).item()
            eval_dict["l2 %d (mesh)" % i] = l2_loss

            # Assuming normalized_distances is your normalized L2 loss values
            # only keep the range of distances between 0 and 0.1
            distances = np.clip(distances, 0, 0.05)

            # Normalize the distances to range [0,1]
            normalized_distances = distances / 0.05
            # Create a linear color map from blue to red
            import matplotlib.pyplot as plt

            cmap = plt.get_cmap("jet")

            # Apply the color map to your normalized_distances
            colors = cmap(normalized_distances)

            colored_pcd.append(colors)

    if not return_colored_pcd:
        return eval_dict
    else:
        return eval_dict, colored_pcd


def eval_correspondences_NN(vertices, pcl_inp, n_pts=10000):
...
def eval_mesh_pc(
    mesh, gt_pc, thresholds=np.linspace(1.0 / 1000, 1, 1000), num_sample=100000
):
