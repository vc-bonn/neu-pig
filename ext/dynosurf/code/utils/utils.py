...
def write_to_mesh(
    ...
    if faces is not None:
        for i in range(faces.shape[0]):
            ...
            mesh.add_face(
                [
                    mesh.vertex_handle(f[0]),
                    mesh.vertex_handle(f[1]),
                    mesh.vertex_handle(f[2]),
                ]
            )
...
def set_color_from_err(errs, max_err=None, min_err=None):
    ...
    for i in range(errs.shape[0]):
        ...
        id = 255 - id
        err_colors.append(
            [colors[id, 0] / 255, colors[id, 1] / 255, colors[id, 2] / 255, 1.0]
        )
...
def Batch_index_select(data, idx):
    return torch.cat(
        [torch.index_select(a, 0, i).unsqueeze(0) for a, i in zip(data, idx)], 0
    )


def Batch_index_select2(data, idx):
    return torch.cat([torch.index_select(a, 0, idx).unsqueeze(0) for a in data], 0)
...
def vis_blending_weight_all(
    ...
    vn_colors = torch.index_select(node_colors, 0, vn_idx.reshape(-1)).reshape(
        points.shape[0], vn_weight.shape[1], 3
    )
